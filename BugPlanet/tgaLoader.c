 ...
 */
int LoadTgaImage(char filename[],unsigned char** ppData,unsigned int* w,unsigned int* h,unsigned int* bpp)
{
...
/*
 *	A function to load a tga file in memory and return the OpenGL texture object
 */
int LoadTgaImageFromMemory(unsigned char* filebuf,unsigned char** ppData,unsigned int* w,unsigned int* h,unsigned int* bpp)
{
	...
	if(filebuf)
	{
		 ...
		 */
		//fread(&header,1,sizeof(tga_header),fp);
		memcpy( &header, filebuf, sizeof( tga_header ) );
		filebuf += sizeof( tga_header );
		...
		*ppData = (unsigned char*)malloc( (*bpp) * header.m_Width * header.m_Height );
		...
		if( header.m_ImageType  == 1 )
		{
			...
			*bpp = ( header.m_ColourMapESize / 8 );
			...
			/* read colour map data */
			memcpy( colourMap, filebuf, header.m_ColourMapSize *(*bpp) );
			filebuf += header.m_ColourMapSize *(*bpp);
			...
			/* read the data in one go, we'll flip it upside down when expanding the colour map data.... */
			//fread( tempptr, 1, header.m_Width * header.m_Height, fp );
			memcpy( tempptr, filebuf, header.m_Width * header.m_Height );
			filebuf += header.m_Width * header.m_Height;
			...
			*ppData = (unsigned char*) malloc ( header.m_Height * header.m_Width * (*bpp)  );
			...
			while( pde != pData )
			{
				...
				if( (byte & 0x80) == 0 )
				{
					...
					for(j=0;j<byte; ++j)
					{
						unsigned char c = /*fgetc(fp)*/ *filebuf;
						//filebuf++;
		...
		else
		{
			...
			{
			case 0:
				printf("[ERROR] The file contains no image data\n");
				break;
			case 3:
				printf("[ERROR] The file contains black & white data which is unsupported\n");
				break;
			case 11:
				printf("[ERROR] The file contains compressed black & white data which is unsupported\n");
				break;
			case 32:
				printf("[ERROR] The file contains compressed color map data which is un-supported\n");
				break;
			case 33:
				printf("[ERROR] The file contains compressed color map data which is un-supported (4pass quad tree process)\n");
				break;
			default:
				printf("[ERROR] The file does not appear to be valid\n");
				break;
